...
//#define Z_DUAL_STEPPER_DRIVERS
  ...
  #define Z_DUAL_ENDSTOPS
	...
	#ifndef Z_DUAL_ENDSTOPS_ADJUSTMENT
 ...
 */
#if ((MOTHERBOARD == BOARD_MELZI_ZONESTAR) && ENABLED(BLTOUCH) || (MODEL_NUMBER == Z6B_WiFi))
#else
#define ADVANCED_PAUSE_FEATURE
  ...
  #define FILAMENT_CHANGE_UNLOAD_FEEDRATE 15  	// Unload filament feedrate in mm/s - filament unloading can be fast
  #define FILAMENT_CHANGE_UNLOAD_LENGTH   PTFE_TUBE_LENGTH // Unload filament length from hotend in mm
                                              // Longer length for bowden printers to unload filament from whole bowden tube,
                                              // shorter length for printers without bowden to unload filament from extruder only,
                                              // 0 to disable unloading for manual unloading
  #define FILAMENT_CHANGE_LOAD_FEEDRATE   10  // Load filament feedrate in mm/s - filament loading into the bowden tube can be fast
  #define FILAMENT_CHANGE_LOAD_LENGTH 	  (PTFE_TUBE_LENGTH*0.9) // Load filament length over hotend in mm
...
/**
* MXING EXTRUDER feature
*
* Add support for auto mixing feature
* [MIXING_COMPENSATION] Add compensate extrusion distance when extrusion length is smaller
* [ENABLE_MIXING_SET_MENUE] Add "set mix rate" on menu 
* [ENABLE_AUTO_MIXING] Add an "auto mixing" function, this function can set 2 kinds of automatic mixing modes:
*     0: disable auto mixing function
*     1: Automatically "mix in" filament from the 2nd extruder according to the printing progress.
*     2: Randomly "mix in" filament from the 2nd extruder according to the printing progress.
**/

#if ENABLED(MIXING_EXTRUDER)  
  #define MIXING_COMPENSATION		//compensate extrusion distance when extrusion length is smaller
  #if (MIXING_STEPPERS == 2)
    #define ENABLE_MIXING_SET_MENUE	//show set mix rate menu
    #if ENABLED(SDSUPPORT)
      #define ENABLE_AUTO_MIXING	//Enable auto mixing function 
      #if ENABLED(ENABLE_AUTO_MIXING)
	  #define	DEFAULT_EXT_MIN		10
	  #define	DEFAULT_EXT_MAX		90
	  #endif
	#endif
...
//#define	ELABLE_MIXING_DEBUG	  //
//#define	ELABLE_MIXING_INFO	  //
#if ENABLED(ELABLE_MIXING_DEBUG)
  #define DEBUG_MIXING	  SERIAL_ECHOLNPAIR
...
#if ENABLED(ELABLE_MIXING_DEBUG) || ENABLED(ELABLE_MIXING_INFO)
  #define INFO_MIXING SERIAL_ECHOLNPAIR
#else
  #define INFO_MIXING
#endif
