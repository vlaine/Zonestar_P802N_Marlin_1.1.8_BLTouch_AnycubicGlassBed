...
void lcd_printPGM_utf(const char *str, uint8_t n=LCD_WIDTH) {
  ...
  static void logo_lines(const char* const extra) {
    int16_t indent = (LCD_WIDTH - 10 - lcd_strlen_P(extra)) / 2;
    lcd.setCursor(indent, 0); lcd.print('\x00'); lcd_printPGM(PSTR( "--------" ));  lcd.write('\x01');
    lcd.setCursor(indent, 1);                    lcd_printPGM(PSTR("|ZONESTAR|"));  lcd_printPGM(extra);
...
FORCE_INLINE void _draw_heater_status(const int8_t heater, const char prefix, const bool blink) {
  ...
  if (prefix >= 0) {
    lcd.print((char)LCD_STR_DEGREE[0]);
    lcd.write(' ');
    if (t2 < 10) lcd.write(' ');
  }
}
...
static void lcd_implementation_status_screen() {
  ...
  lcd.setCursor(0, 0);
    ...
    //
      ...
      #else
        lcd.print((char)LCD_STR_BEDTEMP[0]);
        _draw_heater_status(-1, -1, blink);
      #endif
    ...
    // Hotend 0 Temperature
    //
    _draw_heater_status(0, (char)LCD_STR_THERMOMETER[0], blink);
    ...
    //
      ...
      lcd.setCursor(10, 0);
      #if HOTENDS > 1
        _draw_heater_status(1, (char)LCD_STR_THERMOMETER[0], blink);
      #else
        _draw_heater_status(-1, (char)LCD_STR_BEDTEMP[0], blink);
      #endif
  ...
  //
	...
	//feedrate
    lcd.setCursor(0, 2);
    lcd.write((char)LCD_STR_FEEDRATE[0]);
    lcd.print(itostr3(feedrate_percentage));
    lcd.write('%');

	//Mix rate
	#if ENABLED(MIXING_EXTRUDER) && ENABLED(ENABLE_AUTO_MIXING) && LCD_WIDTH >= 20
    lcd.setCursor(6, 2);
	lcd_printPGM(PSTR("M:"));
    lcd.print(itostr3left(mixing_factor[0]));
	lcd.write('/');
	lcd.print(itostr3left(mixing_factor[1]));
	#else
	char buffer[10];
    duration_t elapsed = print_job_timer.duration();
    uint8_t len = elapsed.toDigital(buffer);

    lcd.setCursor(LCD_WIDTH - len - 8, 2);
    lcd.write((char)LCD_STR_CLOCK[0]);
    lcd_print(buffer);
	#endif//ENABLED(MIXING_EXTRUDER) && ENABLED(ENABLE_AUTO_MIXING) && LCD_WIDTH >= 20
  ...
  lcd.setCursor(0, LCD_HEIGHT - 1);
	...
	lcd.print((char)LCD_STR_THERMOMETER[0]);
	lcd.write(' ');
	lcd.print((char)LCD_STR_UPLEVEL[0]);
	lcd.write(' ');
	lcd.print((char)LCD_STR_REFRESH[0]);
	lcd.write(' ');
	lcd.print((char)LCD_STR_FOLDER[0]);
	lcd.write(' ');
	lcd.print((char)LCD_STR_FEEDRATE[0]);
	lcd.write(' ');
	lcd.print((char)LCD_STR_CLOCK[0]);
	lcd.write(' ');
#else
  #if ENABLED(LCD_PROGRESS_BAR)
    ...
    static void lcd_implementation_hotend_status(const uint8_t row) {
      if (row < LCD_HEIGHT) {
        ...
        _draw_heater_status(active_extruder, (char)LCD_STR_THERMOMETER[0], lcd_blink());
      }
    }
  ...
  #define lcd_implementation_drawmenu_function(sel, row, pstr, data) lcd_implementation_drawmenu_generic(sel, row, pstr, '>', ' ')
    ...
    */
    ...
    void lcd_implementation_ubl_plot(const uint8_t x, const uint8_t inverted_y) {
        ...
        #define _PLOT_X 1
        #define _MAP_X 3
        #define _LABEL(C,X,Y) lcd.setCursor(X, Y); lcd.write(C)
