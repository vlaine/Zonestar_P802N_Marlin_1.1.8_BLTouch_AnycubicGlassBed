...
static void lcd_implementation_init() {
  ...
  #if ENABLED(MKS_12864OLED) || ENABLED(MKS_12864OLED_SSD1306) || ENABLED(ZONESTAR_OLED12864)    
	OUT_WRITE(LCD_PINS_DC,LOW);
	OUT_WRITE(DOGLCD_CS,LOW);
	#if PIN_EXISTS(LCD_PINS_RS)		
...
FORCE_INLINE void _draw_mix_rate(const uint8_t ext, const uint8_t x, const uint8_t y){
  const uint8_t degsize = 6 * (mixing_factor[ext] >= 100 ? 3 : mixing_factor[ext] >= 10 ? 2 : 1); // number's pixel width
  u8g.setPrintPos(x - (18 - degsize) / 2, y); // move left if shorter
  #if MIXING_STEPPERS > 2
  lcd_print(i8tostr3(mixing_factor[ext]));
  if(ext < MIXING_STEPPERS-1)
	  lcd_print(':');
  #else
  lcd_print(i8tostr3(mixing_factor[ext]));
  lcd_print('%');
...
FORCE_INLINE void _draw_mix_mode(const uint8_t mode, const uint8_t x, const uint8_t y){
  const uint8_t degsize = 6 * (mode >= 100 ? 3 : mode >= 10 ? 2 : 1); // number's pixel width
  u8g.setPrintPos(x - (18 - degsize) / 2, y); // move left if shorter
  //Linear
  //Random mix
  switch(mode)
  {
  	case 1: 
  		lcd_printPGM(PSTR(MSG_MIXMODE_LINEAR));
	break;
	
	case 2: 
  		lcd_printPGM(PSTR(MSG_MIXMODE_RANDOM));
	break;

	default:
	case 0: 
  		lcd_printPGM(PSTR(MSG_MIXMODE_MANUAL));
	break;	
  }
  //lcd_printPGM(PSTR("Mode:"));
  //lcd_print(itostr3left(mode));
}
#endif

FORCE_INLINE void _draw_mixrate_status(const bool blink) {
  #if !HEATER_IDLE_HANDLER
    UNUSED(blink);
  #endif
  if (PAGE_UNDER(7)) {
	...
	for(uint8_t i=0; i<MIXING_STEPPERS; i++)
	    _draw_mix_rate(i, x+i*gap, 7);
  }
#if ENABLED(ENABLE_AUTO_MIXING)
  if (PAGE_UNDER(28)) 
	  _draw_mix_mode(auto_mixing_mode, 40, 28);
#endif
}
#endif
...
static void lcd_implementation_status_screen() {
  ...
  void lcd_implementation_drawedit(const char* const pstr, const char* const value=NULL) {
    ...
    static void _drawmenu_sd(const bool isSelected, const uint8_t row, const char* const pstr, const char* filename, char* const longFilename, const bool isDir) {
      ...
      if (isDir) lcd_print((char)LCD_STR_FOLDER[0]);
