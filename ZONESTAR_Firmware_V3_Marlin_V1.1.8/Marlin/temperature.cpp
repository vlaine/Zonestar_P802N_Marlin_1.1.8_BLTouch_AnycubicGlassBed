  ...
  void Temperature::thermal_runaway_protection(Temperature::TRState * const state, millis_t * const timer, const float current, const float target, const int8_t heater_id, const uint16_t period_seconds, const uint16_t hysteresis_degc) {
    ...
    */
    ...
    switch (*state) {
      ...
      case TRStable:
	  	if(current >= EXTRUDE_MINTEMP) break;
		else if ((current >= tr_target_temperature[heater_index] - hysteresis_degc)){
...
void Temperature::isr() {
  ...
  ISR_STATICS(0);
    ...
    if (pwm_count_tmp >= 127) {
      ...
      soft_pwm_count_0 = (soft_pwm_count_0 & pwm_mask) + soft_pwm_amount[0];
        ...
        WRITE_HEATER_BED(soft_pwm_count_BED > pwm_mask ? HIGH : LOW)
		  ...
		  if(Work_Mode == MODE_LASER && laser_TTL_phase)
            WRITE_FAN(soft_pwm_count_fan[0] > pwm_mask ? LOW : HIGH);
		  else
          #endif
      ...
      if (soft_pwm_count_0 <= pwm_count_tmp){ 
	  	#if ENABLED(SPINDLE_LASER_ENABLE)
	    if(Work_Mode == MODE_FFF)
	    #endif
	  	  WRITE_HEATER_0(LOW); 
      ...
      #if HAS_HEATER_BED
        if (soft_pwm_count_BED <= pwm_count_tmp) {			
			WRITE_HEATER_BED(LOW)
		}
      #endif
         ...
         #if ENABLED(SPINDLE_LASER_ENABLE) && (!SPINDLE_LASER_PWM)
		 if(Work_Mode == MODE_LASER && laser_TTL_phase){
		 	if (soft_pwm_count_fan[0] <= pwm_count_tmp) WRITE_FAN(HIGH);
		 }
		 else
         #endif
    ...
    PWM_OFF_ROUTINE(0);
      ...
      if (pwm_count_tmp >= 127) {
        pwm_count_tmp = 0;
		  ...
		  #if ENABLED(SPINDLE_LASER_ENABLE) && (!SPINDLE_LASER_PWM)
  		  if(Work_Mode == MODE_LASER && laser_TTL_phase)
              WRITE_FAN(soft_pwm_count_fan[0] > 0 ? LOW : HIGH);
		  else
		  #endif
      ...
      #if HAS_FAN0	    
        #if ENABLED(SPINDLE_LASER_ENABLE) && (!SPINDLE_LASER_PWM)
	    if(((Work_Mode == MODE_LASER) && laser_TTL_phase && (soft_pwm_count_fan[0] <= pwm_count_tmp)) WRITE_FAN(HIGH);
	    else
        #endif
        if (soft_pwm_count_fan[0] <= pwm_count_tmp) WRITE_FAN(LOW);
      #endif
