 ...
 * M166 - Set auto mixing feature by progress percent when printing from SD card
...
WorkMode Work_Mode = MODE_FFF;
bool laser_TTL_phase = true;
#endif
...
#if ENABLED(ADJUST_Z_MIN_POS)
float z_min_pos = Z_MIN_POS; // Initialized by settings.load()
#endif
...
float cartes[XYZ] = { 0 };
  ...
  uint8_t mixing_factor_buf[MIXING_STEPPERS];
  uint8_t mixing_factor[MIXING_STEPPERS]; // Reciprocal of mix proportion. 0 = off, otherwise <= 100
  #if ENABLED(MIXING_COMPENSATION) 
  int16_t mix_compensation_count[MIXING_STEPPERS];
  #endif
  
  #if MIXING_VIRTUAL_TOOLS > 1
    uint8_t mixing_virtual_tool_mix[MIXING_VIRTUAL_TOOLS][MIXING_STEPPERS];
  #endif

  #if ENABLED(ENABLE_AUTO_MIXING)  
  uint8_t auto_mixing_mode = 0;  
  uint8_t auto_mixing_min = DEFAULT_EXT_MIN;
  uint8_t auto_mixing_max = DEFAULT_EXT_MAX;
  #endif
#endif
      ...
      if (card_eof || n == -1
        ...
        if (card_eof) {
          ...
          if (card.sdprinting)
			...
			#if ENABLED(MIXING_EXTRUDER) && ENABLED(ENABLE_AUTO_MIXING)
			  Reset_auto_mixing();			
			#endif
...
inline void gcode_G92() {
 ...
 /**
  * M451 swich to FFF mode
  * M452 swich to laser mode
  */
  inline void gcode_M451() {
    Work_Mode = MODE_FFF;
  }
  inline void gcode_M452() {
  	const uint8_t value = parser.byteval('S');
	if(value == 0)
		laser_TTL_phase = false;
	else
		laser_TTL_phase = true;
    Work_Mode = MODE_LASER;
  }
...
inline void gcode_M502() {
  ...
  inline void gcode_M600() {
	...
	for(uint8_t i = 0; i<MIXING_STEPPERS; i++){
		mixing_factor_buf[i] = mixing_factor[i];
		mixing_factor[i] = 100/MIXING_STEPPERS;
	}
	DEBUG_MIXING(" mixing_factor[0] = ", mixing_factor[0]);
	DEBUG_MIXING(" mixing_factor[1] = ", mixing_factor[1]);
	#if MIXING_STEPPERS > 2
	DEBUG_MIXING(" mixing_factor[2] = ", mixing_factor[2]);
	#endif
	DEBUG_MIXING(" mixing_factor_buf[0] = ", mixing_factor_buf[0]);
	DEBUG_MIXING(" mixing_factor_buf[1] = ", mixing_factor_buf[1]);
	#if MIXING_STEPPERS > 2
	DEBUG_MIXING(" mixing_factor_buf[2] = ", mixing_factor_buf[2]);
	#endif
	#endif		
    ...
    const float unload_length = parser.seen('U') ? parser.value_axis_units(E_AXIS) : 0
	   ...
	   #if ENABLED(MIXING_EXTRUDER)
	   - (FILAMENT_CHANGE_UNLOAD_LENGTH*MIXING_STEPPERS)
	   #else
    ...
    const float load_length = parser.seen('L') ? parser.value_axis_units(E_AXIS) : 0
	   ...
	   #if ENABLED(MIXING_EXTRUDER)
        + FILAMENT_CHANGE_LOAD_LENGTH*MIXING_STEPPERS
       #else
...
{
	uint8_t i;
	uint16_t mixing_factor_total = 0;	
  	for(i=0; i<MIXING_STEPPERS; i++) 
		mixing_factor_total += mixing_factor_buf[i];
	DEBUG_MIXING(" mixing_factor_buf[0] = ", mixing_factor_buf[0]);
	DEBUG_MIXING(" mixing_factor_buf[1] = ", mixing_factor_buf[1]);
	#if MIXING_STEPPERS > 2
	DEBUG_MIXING(" mixing_factor_buf[2] = ", mixing_factor_buf[2]);
	...
	DEBUG_MIXING(" mixing_factor_buf[3] = ", mixing_factor_buf[3]);
	#endif
	DEBUG_MIXING(" mixing_factor_total = ", mixing_factor_total);
	for(i=0; i<MIXING_STEPPERS-1; i++) 
		mixing_factor[i] = ((uint16_t)mixing_factor_buf[i]*100)/mixing_factor_total;
	mixing_factor_total = 0;
	for(i=0; i<(MIXING_STEPPERS-1); i++) 
		mixing_factor_total += mixing_factor[i];	
	mixing_factor[MIXING_STEPPERS-1] = 100 - mixing_factor_total;
  	DEBUG_MIXING(" mixing_factor[0] = ", mixing_factor[0]);
	DEBUG_MIXING(" mixing_factor[1] = ", mixing_factor[1]);	
	#if MIXING_STEPPERS > 2
	DEBUG_MIXING(" mixing_factor[2] = ", mixing_factor[2]);
	#endif	
	#if MIXING_STEPPERS > 3
	DEBUG_MIXING(" mixing_factor[3] = ", mixing_factor[3]);
	#endif
	for(i=0; i<MIXING_STEPPERS; i++) 
		mixing_factor_buf[i] = mixing_factor[i];
}
  ...
  // The total "must" be 100 (but it will be normalized)
  ...
  void gcode_get_mix() {
	...
	for (uint8_t i = 0; i < MIXING_STEPPERS; i++) {
	  if (parser.seenval(mixing_codes[i])) {
		SBI(mix_bits, i);
		mixing_factor_buf[i] = parser.value_int();			
	  }
	}
  ...
  #endif//ENABLED(DIRECT_MIXING_IN_G1)	
  ...
  inline void gcode_M163() {
    ...
    if (mix_index < MIXING_STEPPERS) {
		#if (MIXING_STEPPERS == 2)
		mixing_factor_buf[mix_index] = parser.intval('P');
		INFO_MIXING(" index = ", mix_index);
		INFO_MIXING(" mixing_factor_buf = ", mixing_factor_buf[mix_index]);
		if(mix_index == 0)
			mixing_factor_buf[1] = 100 - mixing_factor_buf[0];
		else if(mix_index == 1)
			mixing_factor_buf[0] = 100 - mixing_factor_buf[1];			
		normalize_mix();
		#else
		mixing_factor_buf[mix_index] = parser.intval('P');
		INFO_MIXING(" index = ", mix_index);
    ...
    inline void gcode_M164() {
	  ...
	  INFO_MIXING(" tool_index = ", tool_index);
	  INFO_MIXING(" mixing_virtual_tool_mix[0] = ", mixing_virtual_tool_mix[0]);
	  INFO_MIXING(" mixing_virtual_tool_mix[1] = ", mixing_virtual_tool_mix[1]);
	  #if MIXING_STEPPERS > 2
	  INFO_MIXING(" mixing_virtual_tool_mix[2] = ", mixing_virtual_tool_mix[2]);
	  #endif	
	  #if MIXING_STEPPERS > 3
	  INFO_MIXING(" mixing_virtual_tool_mix[3] = ", mixing_virtual_tool_mix[3]);
	  #endif
    }
  #endif
     ...
     *       All factors together must add up to 100.
     *
    ...
    /**
     * M169: Set auto mixed
     *       Mode: set auto mix mode, value is from 0~3
     *       Factors that are left out will be set to 0.
     *       All factors together must add up to 1.0.
     *
     * S[0~2]   Mix mode: 0:Disable 1:Linear mixing 2:Random mixing
     * m[0~100] Min mix factor for extruder 1
     * M[0~100] Max mix factor for extruder 1
     *
     */
    inline void gcode_M169() {
      int16_t auto_mix_mode = 0;
	  int16_t auto_mix_min = DEFAULT_EXT_MIN;
	  int16_t auto_mix_max = DEFAULT_EXT_MAX;
	  if(parser.seenval('S'))
      	auto_mix_mode = parser.intval('S');	  
	  if(parser.seenval('m'))
	  	auto_mix_min = parser.intval('m');
	  if(parser.seenval('M'))
	  	auto_mix_max = parser.intval('M');
	  if((auto_mix_mode < 0 || auto_mix_mode > 2) || (auto_mix_min < 0) || (auto_mix_min > 100) || (auto_mix_max < 0) || (auto_mix_max > 100))
	  	return;
      auto_mixing_mode = auto_mix_mode;
	  auto_mixing_min = auto_mix_min;
	  auto_mixing_max = auto_mix_max;
	  INFO_MIXING(" auto_mixing_mode = ", auto_mixing_mode);
	  INFO_MIXING(" auto_mixing_min = ", auto_mixing_min);
	  INFO_MIXING(" auto_mixing_max = ", auto_mixing_max);
    }
  #endif//ENABLED(ENABLE_AUTO_MIXING)
...
#if ENABLED(MIXING_EXTRUDER) && ENABLED(ENABLE_AUTO_MIXING) && ENABLED(SDSUPPORT)
uint8_t old_progress_bar_percent = 0;
void Reset_auto_mixing(){
	if(auto_mixing_mode){
		old_progress_bar_percent = 0;
		auto_mixing_mode = 0;
		auto_mixing_min = DEFAULT_EXT_MIN;
		auto_mixing_max = DEFAULT_EXT_MAX;
		mixing_factor_buf[0] = 100;
		for(uint8_t i=1; i< MIXING_STEPPERS; i++)
			mixing_factor_buf[i] = 0;
		normalize_mix();
	}
}

void auto_mixing_preocess(uint8_t percent){
	if(percent < old_progress_bar_percent)
		old_progress_bar_percent = percent;
	if((auto_mixing_mode > 0) && (percent - old_progress_bar_percent > 0))
	{
		old_progress_bar_percent = percent;
		switch(auto_mixing_mode)
		{
			case 1:
			mixing_factor_buf[0] = ((auto_mixing_max - auto_mixing_min)*(100-percent)/100) + auto_mixing_min;
			mixing_factor_buf[1] = 100 -mixing_factor_buf[0];
			for(uint8_t i=2; i<MIXING_STEPPERS; i++)		
				mixing_factor_buf[i] = 0;
			normalize_mix();
			break;
			...
			case 2:
			#if (MIXING_STEPPERS == 2)
			mixing_factor_buf[0] = random(100);
			mixing_factor_buf[1] = 100 -mixing_factor_buf[0];
			#else
			for(uint8_t i=0; i<MIXING_STEPPERS; i++)
				mixing_factor_buf[i] = random(100);
			#endif
			normalize_mix();			
			break;
		...
		INFO_MIXING("progress_bar_percent = ",percent);
		INFO_MIXING("mixing_factor_buf[0] = ",mixing_factor_buf[0]);
		INFO_MIXING("mixing_factor_buf[1] = ",mixing_factor_buf[1]);
		INFO_MIXING("mixing_factor[0] = ",mixing_factor[0]);
		INFO_MIXING("mixing_factor[1] = ",mixing_factor[1]);
	}
}
#endif
...
#if HAS_FIL_RUNOUT2
#define IS_FIL_RUN_OUT ((READ(FIL_RUNOUT_PIN) == FIL_RUNOUT_INVERTING) || (READ(FIL_RUNOUT2_PIN) == FIL_RUNOUT_INVERTING))
#else
#define IS_FIL_RUN_OUT (READ(FIL_RUNOUT_PIN) == FIL_RUNOUT_INVERTING)
#endif

  #if ENABLED(FILAMENT_RUNOUT_SENSOR)
    if ((IS_SD_PRINTING || print_job_timer.isRunning()) && (IS_FIL_RUN_OUT))
  ...
  #if ENABLED(MIXING_EXTRUDER) && ENABLED(ENABLE_AUTO_MIXING) && ENABLED(SDSUPPORT)
  if(IS_SD_PRINTING)
    auto_mixing_preocess(card.percentDone());
  #endif
...
void setup() {
  ...
  lcd_init();
    ...
    for (uint8_t t = 0; t < MIXING_VIRTUAL_TOOLS && t < MIXING_STEPPERS; t++)
      for (uint8_t i = 0; i < MIXING_STEPPERS; i++)
        mixing_virtual_tool_mix[t][i] = (t == i) ? 100 : 0;

    // Remaining virtual tools are 100% filament 1
    #if MIXING_STEPPERS < MIXING_VIRTUAL_TOOLS
      for (uint8_t t = MIXING_STEPPERS; t < MIXING_VIRTUAL_TOOLS; t++)
        for (uint8_t i = 0; i < MIXING_STEPPERS; i++)
          mixing_virtual_tool_mix[t][i] = (i == 0) ? 100 : 0;
    #endif
	...
	#if ENABLED(MIXING_COMPENSATION) 
	for (uint8_t i = 0; i < MIXING_STEPPERS; i++)
		mix_compensation_count[i] = 0;
	#endif
	
	// Initialize auto mixing	
	mixing_factor_buf[0] = 100;
	for (uint8_t i = 1; i < MIXING_STEPPERS; i++)
		mixing_factor_buf[i] = 0;		
	#if ENABLED(ENABLE_AUTO_MIXING)
