...
void Planner::check_axes_activity() {
  ...
  if (blocks_queued()) {
    ...
    #else
      #if HAS_FAN0
	    #if ENABLED(SPINDLE_LASER_ENABLE) && (!SPINDLE_LASER_PWM)
		if(Work_Mode == MODE_LASER && laser_TTL_phase)
			analogWrite(FAN_PIN, 255 - CALC_FAN_SPEED(0));	
		else
		#endif
...
void Planner::_buffer_steps(const int32_t (&target)[XYZE], float fr_mm_s, const uint8_t extruder) {
  ...
  // For a mixing extruder, get a magnified step_event_count for each
    ...
    for (uint8_t i = 0; i < MIXING_STEPPERS; i++){
	  block->mix_event_count[i] = (mixing_factor[i] * block->steps[E_AXIS])/100;	 
	  
	  #if ENABLED(MIXING_COMPENSATION)
	  mix_compensation_count[i] += (mixing_factor[i] * block->steps[E_AXIS])%100;
	  if(abs(mix_compensation_count[i]) > 100){
	  	block->mix_event_count[i] += mix_compensation_count[i]/100;
		mix_compensation_count[i] -= (mix_compensation_count[i]/100)*100;
	  }
	  DEBUG_MIXING(" mix_compensation_count[0] = ", mix_compensation_count[0]);
	  DEBUG_MIXING(" mix_compensation_count[1] = ", mix_compensation_count[1]);
	  #endif
    }
	DEBUG_MIXING(" mixing_factor[0] = ", mixing_factor[0]);
	DEBUG_MIXING(" mixing_factor[1] = ", mixing_factor[1]);
	DEBUG_MIXING(" block->steps[X_AXIS] = ", block->steps[X_AXIS]);
	DEBUG_MIXING(" block->steps[Y_AXIS] = ", block->steps[Y_AXIS]);
	DEBUG_MIXING(" block->steps[Z_AXIS] = ", block->steps[Z_AXIS]);
	DEBUG_MIXING(" block->steps[E_AXIS] = ", block->steps[E_AXIS]);
	DEBUG_MIXING(" block->step_event_count = ", block->step_event_count);	
	DEBUG_MIXING(" block->mix_event_count[0] = ", block->mix_event_count[0]);
	DEBUG_MIXING(" block->mix_event_count[1] = ", block->mix_event_count[1]);
  #endif
...
void Planner::buffer_segment(const float &a, const float &b, const float &c, const float &e, const float &fr_mm_s, const uint8_t extruder) {
  ...
  if (!blocks_queued()) {
	...
	#if ENABLED(MIXING_EXTRUDER)
	DEBUG_MIXING(" _buffer_steps{1} between=", between);
	DEBUG_MIXING(" fr_mm_s = ", fr_mm_s);
	DEBUG_MIXING(" extruder = ", extruder);
	#endif
	...
	#if ENABLED(MIXING_EXTRUDER)
	DEBUG_MIXING(" _buffer_steps{2} target=", target);
	DEBUG_MIXING(" fr_mm_s = ", fr_mm_s);
	DEBUG_MIXING(" extruder = ", extruder);
	#endif	
	...
	#if ENABLED(MIXING_EXTRUDER)
	DEBUG_MIXING(" _buffer_steps{3} target=", target);
	DEBUG_MIXING(" fr_mm_s = ", fr_mm_s);
	DEBUG_MIXING(" extruder = ", extruder);
	#endif	  	
    _buffer_steps(target, fr_mm_s, extruder);
