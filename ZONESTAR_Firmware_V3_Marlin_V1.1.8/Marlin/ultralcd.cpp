...
void kill_screen(const char* lcd_msg) {
  ...
  void lcd_Apply_mix_ratio_callback(){
	normalize_mix();
  }
  void lcd_set_mix_ratio(){
		START_MENU();
    	MENU_BACK(MSG_TUNE);
		MENU_MULTIPLIER_ITEM_EDIT(int8, MSG_E1_PERCENT, &mixing_factor_buf[0], 0, 100);
		MENU_MULTIPLIER_ITEM_EDIT(int8, MSG_E2_PERCENT, &mixing_factor_buf[1], 0, 100);
		#if (MIXING_STEPPERS > 2)
		MENU_MULTIPLIER_ITEM_EDIT(int8, MSG_E3_PERCENT, &mixing_factor_buf[2], 0, 100);
		#endif
	  	#if (MIXING_STEPPERS > 3)
		MENU_MULTIPLIER_ITEM_EDIT(int8, MSG_E4_PERCENT, &mixing_factor_buf[3], 0, 100);
	  	#endif
		MENU_ITEM(function, MSG_APPLY_MIXRATIO, lcd_Apply_mix_ratio_callback);		
    	END_MENU();
  }
  
  #if ENABLED(ENABLE_AUTO_MIXING)      
	void set_mixing_mode_callback(){
	DEBUG_MIXING(" auto_mixing_mode = ", auto_mixing_mode);
	DEBUG_MIXING(" auto_mixing_min = ", auto_mixing_min);
	DEBUG_MIXING(" auto_mixing_max = ", auto_mixing_max);
	mixing_factor_buf[0] = auto_mixing_min;
	mixing_factor_buf[1] = 100 - auto_mixing_min;
	#if (MIXING_STEPPERS > 2)
    ...
    void lcd_set_auto_mix_mode(){
		START_MENU();
    	MENU_BACK(MSG_TUNE);
		MENU_ITEM_EDIT_CALLBACK(int8, MSG_SET_AUTO_MIXRATE, &auto_mixing_mode, 0,2,set_mixing_mode_callback);
		if(auto_mixing_mode > 0){
			MENU_ITEM_EDIT_CALLBACK(int8, MSG_SET_AUTOMIX_E2MIN, &auto_mixing_min, 0, 100, set_mixing_mode_callback);
			MENU_ITEM_EDIT_CALLBACK(int8, MSG_SET_AUTOMIX_E2MAX, &auto_mixing_max, 0, 100, set_mixing_mode_callback);
		}
    	END_MENU();
    }
  #endif// ENABLE_AUTO_MIXING

  #endif//ENABLED(MIXING_EXTRUDER)
  ...
  void lcd_tune_menu() {
    ...
    //
    #if ENABLED(MIXING_EXTRUDER) && ENABLED(ENABLE_AUTO_MIXING)
		MENU_ITEM(submenu, MSG_SET_AUTO_MIXRATE, lcd_set_auto_mix_mode);	    
    #endif
  ...
  void lcd_cooldown() {
    ...
    void _lcd_goto_next_corner() {
      ...
      switch (bed_corner) {
        case 0:
          current_position[X_AXIS] = X_MIN_BED + DISTANCE_TO_CORNER_X;
          current_position[Y_AXIS] = Y_MIN_BED + DISTANCE_TO_CORNER_Y;
          break;
        case 1:
          current_position[X_AXIS] = X_MAX_BED - DISTANCE_TO_CORNER_X;
          break;
        case 2:
          current_position[Y_AXIS] = Y_MAX_BED - DISTANCE_TO_CORNER_Y;
          break;
        case 3:
          current_position[X_AXIS] = X_MIN_BED + DISTANCE_TO_CORNER_X;
          break;
  ...
  void lcd_control_motion_menu() {
    ...
    #elif HAS_BED_PROBE
      MENU_ITEM_EDIT(float32, MSG_ZPROBE_ZOFFSET, &zprobe_zoffset, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX);
	#elif ENABLED(ADJUST_Z_MIN_POS)
	  MENU_ITEM_EDIT(float32, MSG_ZPROBE_ZOFFSET, &z_min_pos, -10, 10);
    #endif
	
	//Dual Z offset
	#if ENABLED(Z_DUAL_STEPPER_DRIVERS)
	  MENU_ITEM_EDIT(float32, MSG_ZENDSTOP_ADJ, &z_endstop_adj, -2, 2);
	#endif
   ...
   */
    ...
    void lcd_info_board_menu() {
      ...
      STATIC_ITEM(LCDSCREEN_NAME, true);                           		// LCD screen
    ...
    void lcd_info_printer_menu() {
      ...
      STATIC_ITEM(STRING_FIRMWARE_VERSION, true, true);                // Firmware Version
	  ...
	  #if ENABLED(MIXING_EXTRUDER)
	  STATIC_ITEM(MSG_INFO_EXTRUDERS ": " STRINGIFY(MIXING_STEPPERS), true); // Extruders: 2
	  #else
    ...
    void lcd_info_menu() {
	  ...
	  if(fanSpeeds[0] == 157)
	  {
			...
			MENU_ITEM(gcode, MSG_FAST_TO_TOP, PSTR(
				"M203 X150 Y150 Z16\n"
				"G0 X" STRINGIFY(X_BED_SIZE) " Y" STRINGIFY(Y_BED_SIZE) " Z"STRINGIFY(Z_MAX_POS) " F960\n"
				"M203 X150 Y100 Z8\n"
			));
			//
			MENU_ITEM(gcode, MSG_FAST_TO_BOTTOM, PSTR(
				"M203 X150 Y150 Z16\n"
				"G28 X0\n"
				"G0 Z0 F960\n"
				"M203 X150 Y100 Z8\n"
			...
			//
			MENU_ITEM(gcode, MSG_AUTO_UPDOWN, PSTR(
				"M203 X150 Y150 Z16\n"
				"G0 X" STRINGIFY(X_BED_SIZE) " Y" STRINGIFY(Y_BED_SIZE) " Z"STRINGIFY(Z_MAX_POS) " F960\n"
				"G0 X0 Y0 Z10 F960\n"
				"G0 X300 Y300 Z400 F960\n"
				"G0 X0 Y0 Z10 F960\n"
				"G0 X" STRINGIFY(X_BED_SIZE) " Y" STRINGIFY(Y_BED_SIZE) " Z"STRINGIFY(Z_MAX_POS) " F960\n"
				"G0 X0 Y0 Z10 F960\n"
				"G0 X" STRINGIFY(X_BED_SIZE) " Y" STRINGIFY(Y_BED_SIZE) " Z"STRINGIFY(Z_MAX_POS) " F960\n"
			...
			//
			MENU_ITEM(gcode, MSG_HEATER_TEST, PSTR(				
	          	"M104 S260\n"
	          	"M140 S115\n"
	          	"M106 S0\n"
	        ));	    
    ...
    inline bool handle_adc_keypad() {
      ...
      if (buttons_reprapworld_keypad) {
        ...
        if (encoderDirection == -1) { // side effect which signals we are inside a menu
		  ...
		  #if DISABLED(ZONESTAR_LCD2004_ADCKEY)
