...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::save() {
    ...
    float z_offset_buf = 0;
    #if ENABLED(ADJUST_Z_MIN_POS)
	  z_offset_buf = z_min_pos;
	#elif HAS_BED_PROBE
      z_offset_buf = zprobe_zoffset;      
    #endif
	EEPROM_WRITE(z_offset_buf);
  ...
  bool MarlinSettings::load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
	  ...
	  //Z offset
      const float z_offset_buf;
	  EEPROM_READ(z_offset_buf);
	  #if ENABLED(ADJUST_Z_MIN_POS)
	    z_min_pos = z_offset_buf;
	  #elif HAS_BED_PROBE
        zprobe_zoffset = z_offset_buf;
	  #endif
